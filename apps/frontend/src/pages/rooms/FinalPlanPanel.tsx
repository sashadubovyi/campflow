...
export function FinalPlanPanel({ roomId }: Props) {
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-center text-forest-500 font-body text-sm animate-pulse">
        {t('common.loading')}
      </div>
    );
  }
  ...
  if (isEmpty) {
    return (
      <div className="p-6 text-center text-forest-500 font-body">
        <p className="text-3xl mb-3">📋</p>
        <p className="font-display text-base text-forest-900 mb-1">{t('polls.finalPlan.empty')}</p>
        <p className="text-xs text-forest-700">{t('polls.finalPlan.emptyHint')}</p>
      </div>
    );
  }
...
function Section({
  ...
  return (
    <div>
      <h3 className="font-body text-xs font-semibold text-forest-500 uppercase tracking-widest px-2 mb-2">
        {icon} {title}
      </h3>
...
function DecisionCard({ item }: { item: FinalPlanItem }) {
  const { t } = useTranslation();
  return (
    <div className="bg-white rounded-xl border border-forest-100 p-3">
...
function LocationCard({ item }: { item: FinalPlanItem }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-forest-100 p-3">
      <p className="text-sm font-medium text-forest-900">📍 {item.title}</p>
      {item.address && (
        <p className="text-[10px] text-forest-500 mt-0.5 truncate">{item.address}</p>
      )}
      {item.latitude !== null && item.longitude !== null && (
        <a
          ...
          className="text-[10px] text-forest-600 hover:text-forest-900 transition mt-1 inline-block"
        >
  ...
  return (
    <div className="bg-white rounded-xl border border-forest-100 p-3">
      ...
      <div className="flex items-center justify-between mt-1.5">
        {item.assignee ? (
          <div className="flex items-center gap-1.5">
            <Avatar
              ...
              size={18}
            />
            <span className="text-xs text-forest-700">{item.assignee.fullName}</span>
          </div>
        ) : (
        ...
        <span className="text-[10px] text-forest-500">{relativeTime(item.approvedAt)}</span>
      </div>
    </div>
  );
}
